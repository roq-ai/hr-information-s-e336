...
import { getHrManagers } from 'apiSdk/hr-managers';
...
function DashboardPage() {
  ...
  const hrManagerStat = useSWR<PaginatedInterface<HrManagerInterface>>(
    () => `/hr-managers`,
    () => getHrManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hr Managers',
      result: hrManagerStat?.data?.totalCount,
